...
(deftest count-lines-test
  (testing "Vanilla line count test"
    (let [expected-count 3]
      ...
      (is (= (analyzer/count-lines (analyzer/get-lines test-file-name)) expected-count)))))
...
(deftest avg-line-length-test
  (testing "Vanilla average line length test"
    (let [expected-result 3.0]
      ...
      (is (= (analyzer/avg-line-length (analyzer/get-lines test-file-name)) expected-result)))))
...
(deftest count-non-reserved-chars-test
  (testing "Vanilla count-non-reserved-chars count test"
    (let [reserves #{\( \)}
      ...
      (is (= (analyzer/count-non-reserved-chars reserves (analyzer/get-lines test-file-name)) expected-result)))))
...
(deftest count-non-reserved-keywords-test
  (testing "Vanilla count-non-reserved-keywords"
    (let [reserved-words #{"require" "defn" "def" "let" "recur" "loop"}
          expected-result 5]
      (spit test-file-name test-file-content4)
      (is (= (analyzer/count-non-reserved-keywords reserved-words (analyzer/get-lines test-file-name)) expected-result)))))
